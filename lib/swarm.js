//  S W A R M: I D E N T I F I E R S  A N D  S P E C I F I E R S

/*  ASCIIZE refac
 *  Objectives:
 *      * clean readable ascii message/id format
 *      * compact ids as a local optimization
 *      * simpler method sigs: (ke, value, other stuff)
 *  change id
 *      /Class#id.field!time+seq&author-ssn = value
 *  base map
 *      (!time+seq&author-ssn)*
 *      max time+seq for every author-ssn, excluding time + THRESHOLD < max time
 *  sigs
 *      bundle sig
 *          { /Class#id.field!time+seq&author-ssn : value }
 *      object set/on sig
 *          field, value, { field : !time+seq&author-ssn }, src
 *          { field: value }, { field : !time+seq&author-ssn }, src
 *
 *  todos
 *      * handshake&time
...
var reTimestamp = /(\d{4})(\d\d)(\d\d)_(\d\d)(\d\d)(\d\d)/;
Spec2.ts2date = function (ts) {
    var m = ts.match(reTimestamp);
    if (!m) throw new Error('malformed timestamp: '+ts);
    var iso = m[1]+'-'+m[2]+'-'+m[3]+'T'+m[4]+':'+m[5]+':'+m[6];
    return new Date(iso);  // TODO timezone
};
...
var SwarmObjectProto = {
    ...
    createId: function(quant) {
        var lstnall = this._lstn.constructor===Array ? this._lstn[0] : this._lstn[''];
        return lstnall && lstnall.constructor===Peer ? lstnall.createId(quant) : new ID(quant);
    },
...
function Pipe (sink,host,opts) {
    ...
    self.serialize = self.opts.serialize || function (obj) {
        return JSON.stringify(obj);
    };
    self.deserialize = self.opts.deserialize || function (str) {
...
Pipe.prototype.parseHandshake = function ph (spec,specme) {
    ...
    if (hs) {
        this.bundle [ Spec.wireNames['Peer']+this.host._id ] = spec.toString();
        self.sendBundle();
    }
};
    ...
    if (self.timeout===-1)
        self.sendBundle();
            ...
            self.timer = setTimeout(function(){
                self.sendBundle();
                self.timer = null;
            }, gap>self.timeout ? self.politeWait : self.timeout-gap );
};
...
function Plumber (host,urlList,opts) {
    this.host = host;
    this.opts = opts || {};
...
Plumber.prototype.connect = function (uri) {
    ...
    socket.addEventListener(vocab.open||'open',function(err){
        ...
        pipe = new Pipe(socket,self.host,self.opts);
